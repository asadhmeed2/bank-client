...
function AddUser() {
  ...
  const inputHandler = (e) => {
    const tempUser = { ...user };
    if (
      (e.target.name === "cash" || e.target.name === "credit") &&
      ...
      messageRef.current.textContent =
        "Error cash or credit must be a number: ";
    }
  };
  const onFormSubmit = () => {
    if (Number.isNaN(Number(user.cash)) && Number.isNaN(Number(user.credit))) {
      //check if the amount is a number
      messageRef.current.textContent =
        "Error cash or credit are not a number: ";
      return;
    }
    messageRef.current.textContent = "";
    axios
      .post("https://bank-server-demo.herokuapp.com/users", user)
      .then((response) => {
  ...
  return (
    <div className="addUser">
      ...
      <div className="addUserLogo">
        <h1>
          <span className="Add-h1">Add</span>{" "}
          <span className="New-h1">New</span>{" "}
          <span className="user-h1">User</span>{" "}
        </h1>
      </div>
    </div>
  );
